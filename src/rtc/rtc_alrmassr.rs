...
pub enum Maskss {
    ...
    B0x0 = 0,
    #[doc = "1: SS\\[31:1\\]
are dont care in Alarm A comparison. Only SS\\[0\\]
is compared."]
    B0x1 = 1,
}
...
in RTC_SSR) is running from 0xFFFF1FFFF to RTC_ALRABINR.SS\\[31:0\\]
value and is automatically reloaded with 0xFFFF1FFFF one ck_apre cycle after reaching RTC_ALRABINR.SS\\[31:0\\]."]
    B0x1 = 1,
}
...
in the RTC_ALRABINR, and so can also be read or written through RTC_ALRABINR."]
    ...
    #[inline(always)]
    #[must_use]
    pub fn ssclr(&mut self) -> SsclrW<RtcAlrmassrSpec> {
        SsclrW::new(self, 31)
    }
}
